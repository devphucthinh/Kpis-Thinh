...
public static class DependencyInjection
{
    public static IServiceCollection AddKpiPostgres(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        if (!string.IsNullOrWhiteSpace(connection)) { services.AddDbContext<KpiDbContext>(options => options.UseNpgsql(connection)); services.AddScoped<Stores.PostgresKpiDefinitionStore>(); services.AddScoped<Kpi.Application.Persistence.IKpiDefinitionPersistence>(sp => sp.GetRequiredService<Stores.PostgresKpiDefinitionStore>()); }
        return services;
    }
}
