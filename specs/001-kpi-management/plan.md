...
| One Deterministic Verification Path | Adds every setup, migration, formatting, lint, test, browser, and full-check command to `.harness/harness.json`; no parallel verification workflow. | Pass |
...
The current repository contains the implemented baseline and a reviewed `KpiMigrationRunner`, but the runner is not yet reachable from a canonical command and the Web host can still fall back to InMemory storage. The proposed dependency direction remains compatible with `docs/architecture.md`: application code never depends on the harness, while the harness invokes the dedicated migrator composition root. This migration slice must close that wiring gap without moving schema policy into Web or scripts.
...
├── Kpi.Migrator/                   explicit local/test migration entrypoint
...
`Kpi.Domain` has no reference to application framework, database or delivery projects. `Kpi.Migrator` is a composition-root tool only: it may reference Infrastructure and configuration libraries, but no Web/UI code. Test projects reference only the modules they exercise; PostgreSQL and Web tests use the outer boundaries rather than testing private methods.

The command contract for the new entrypoint is recorded in
...
Kpi.Migrator ─── explicit command-line composition root
        │
        └── Kpi.Infrastructure.Postgres ─── PostgreSQL
```
...
| `Kpi.Infrastructure.Postgres` | EF mappings, PostgreSQL migrations, storage-port adapters, constraints, transaction implementation, database permission setup. | Lifecycle policy or formula semantics; command-line argument parsing. | Kpi.Application + Kpi.Domain + persistence packages. |
| `Kpi.Migrator` | Loads non-secret target settings plus an external migration connection, validates the local/test allow-list, and invokes the Infrastructure migration runner. | Web startup, product seeding, lifecycle policy, arbitrary SQL, or runtime credentials. | Kpi.Infrastructure.Postgres plus .NET hosting/configuration libraries. |
...
| `FormulaEvaluator.Evaluate` | Produces Decimal/Boolean success or structured Failure from a compiled formula and resolved non-null Evaluation Inputs; required variables and formula-referenced optional variables must have an explicit input or compatible default, while unreferenced optional variables may be omitted. | Domain unit test with deterministic budget/time source. |
...
| `migrate` harness action | Applies the ordered manifest to an explicitly allowed local/test database, reports applied/skipped IDs, and leaves Web startup read-only with respect to schema. | Harness contract test plus PostgreSQL migration integration test. |
...
Formula input resolution is explicit: a required variable without an input or
compatible non-null default fails before evaluation; an optional variable is
omittable only when the compiled formula does not reference it; a referenced
optional variable without an input or compatible default returns the stable
missing-input Failure and never becomes Null.
...
.\harness.cmd migrate
        │
        ▼
scripts/harness.ps1 (declared migrate step)
        │
        ▼
Kpi.Migrator (configuration + target allow-list)
        │
        ▼
Kpi.Infrastructure.Postgres.Migrations.KpiMigrationRunner
        │  one transaction, manifest order, ledger/checksum checks
        ▼
PostgreSQL local/test database
```

The command receives a privileged migration connection from
`ConnectionStrings:KpiMigration`, validates the database name before opening a
transaction, and never accepts arbitrary SQL or a caller-supplied database
override. It applies only the reviewed `KpiMigrationManifest`, writes one
ledger row per applied ID, and returns non-zero on configuration, checksum,
permission, SQL, or transaction failure. The Web host uses
`ConnectionStrings:KpiRuntime` and does not call this flow.
...
Keep schema evolution facts in a small relational migration ledger:

- `kpi_schema_migrations(id, checksum, applied_at)` records each applied manifest
  entry once;
- the ledger is created by the migrator inside the same transaction as the first
  product migration;
- a previously applied ID with a different checksum is a hard failure, not an
  implicit rewrite or re-run.
...
- Separate a privileged schema-migration credential from the limited runtime credential. The Web host reads `ConnectionStrings:KpiRuntime`; the migrator reads `ConnectionStrings:KpiMigration`. Store both only in user secrets/environment variables, never `.env` or Git.
...
- The explicit `migrate` harness action creates/updates schema only for explicitly configured local/test databases. `bootstrap` validates non-secret configuration and dependencies but does not mutate a database. Integration tests use a distinct `kpi_lab_test` database and validate its name before any drop/recreate step.
- The migrator accepts only a connection whose database name exactly equals `Kpi:DatabaseName` or `Kpi:TestDatabaseName`; it rejects arbitrary database names before opening a transaction. It runs the ordered manifest in one transaction per invocation, is safe to repeat, and never performs down migrations or `EnsureCreated`.
- The Web host does not invoke the migrator on startup. `run-kpi.bat` remains a host launcher; the documented local sequence is `bootstrap`, `migrate`, then launch.
...
PostgreSQL integration tests use an explicit opt-in profile. The default
`harness.cmd test` path remains safe without database credentials and reports a
deterministic integration-test skip. Setting `KPI_POSTGRES_TESTS=1` requires
both `ConnectionStrings:KpiRuntime` and `ConnectionStrings:KpiMigration`,
`Kpi:TestDatabaseName` equal to `kpi_lab_test`, and the declared local/test
target guard; missing configuration fails with a stable configuration error
instead of silently skipping. The enabled profile runs migration-ledger,
rollback, role, JSONB round-trip, concurrency, and Web persistence tests
...
|---|---|
| `bootstrap` | During scaffolding, perform the one-time deterministic lockfile initialization through this declared harness action, review and commit every project lockfile, then enforce `dotnet restore --locked-mode` on all recurring runs. It also installs the Playwright browser when absent and checks required non-secret local/test configuration; it does not mutate PostgreSQL. |
| `migrate` | Validate the migration-only connection and declared local/test database name, build/run `Kpi.Migrator` without restore, apply the ordered forward-only manifest transactionally, and report applied/skipped IDs. This is the only schema-writing command exposed to contributors and CI setup jobs. |
| `format` | Verify formatting. |
| `lint` | Formatting, static analysis and build without restore after locked bootstrap has succeeded. |
| `test` | Existing branch-policy and integration-guide tests plus `tests/harness/migration-command.tests.ps1`, the intended Domain, Application, PostgreSQL integration and browser smoke projects, all without restore; harness wiring proves every declared project and migration contract test is actually executed before the first behavior RED test. |
| `check` | Contract check, locked bootstrap, lint, and test in that order; it never writes application schema. Without `KPI_POSTGRES_TESTS=1` it remains safe when no local PostgreSQL credentials are available; the enabled profile fails clearly when required settings are missing. |

No command is added outside `.harness/harness.json`; CI continues to execute the PowerShell harness entrypoint. The one-time lockfile initialization is a bootstrap transition, not a user-facing parallel restore command; after reviewed lockfiles exist, bootstrap must never rewrite them. `migrate` is an explicit, separately auditable database action rather than a hidden side effect of `bootstrap`, `check`, or Web startup.

The task decomposition must name observable work for all four recurring prerequisites rather than hiding them behind “test setup”: pinned Playwright browser provisioning when absent, required non-secret local/test configuration validation, explicit migration configuration/target validation, and safe application of declared local/test migrations. The first end-to-end discovery witness depends on browser provisioning, and every persistence migration slice extends the same migration manifest consumed by `migrate`.
...
| .NET console migrator | Explicit, cross-platform composition root for the reviewed migration runner. | Local/test schema setup, migration ledger output, role separation. | Web-startup migration hides writes and couples database setup to UI availability. |
...
1. **Scaffold and harness proof** — pin SDK/packages, create solution/module/test boundaries plus the `Kpi.Migrator` entrypoint and architecture ADR, initialize reviewed package lockfiles through `bootstrap`, switch recurring bootstrap to locked restore, wire `lint`/`test`/`check` and explicit `migrate` to the intended projects, and prove the canonical harness executes them. Only then introduce the first behavior RED test.
...
8. **Acceptance and operational finish** — add a delivery-level end-to-end workflow only after each component behavior is green; complete harness/CI parity, migration/operator guide, measurable acceptance evidence, and the required final constitution recheck.
