...
- 2026-08-13 opt-in PostgreSQL verification: `./harness.cmd migrate` targeted allow-listed `kpi_lab_test` with all 11 declared migrations already applied; the complete `Kpi.IntegrationTests` assembly passed 72/72 with 0 failed and 0 skipped.
- 2026-08-13 final traceability rerun: foundational test display names retain exact `FR-xxx` identifiers across decision, catalog, persistence, migration, composition, Problem Details, transaction, and current-facts coverage; the same elevated harness and opt-in PostgreSQL commands remained green after this metadata-only change.
